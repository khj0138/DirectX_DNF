...
namespace hj
{
	Dungeon_Hysmar_Entrance::Dungeon_Hysmar_Entrance()
	{
	...
	void Dungeon_Hysmar_Entrance::Initialize()
	{
		SetMinMax(Vector2(00.0f, 0.0f), Vector2(1100.0f, 350.0f));
		...
		GameObject* gate = new GameObject();
		{
			...
			gate->GetComponent<Transform>()->SetVirtualZ(500.0f);
			...
			portal->SetDestination(L"Dungeon_Entrance_2", Vector2(500.0f, 100.0f));
		...
		gate = new GameObject();
		{
			...
			mr->SetMesh(Resources::Find<Mesh>(L"RectMesh"));
			mr->SetMaterial(Resources::Find<Material>(L"SpriteAnimationMaterial"));
			gate->GetComponent<Transform>()->SetPosition(Vector3(550.0f, 0.0f, 450.0f));
			gate->GetComponent<Transform>()->SetVirtualZ(00.0f);
			...
			gate->AddComponent<CommonPortalScript>();
			PortalScript* portal = gate->AddComponent<PortalScript>();
			portal->SetDestination(L"Dungeon_Hysmar", Vector2(830.0f, 300.0f));
			gate->SetState(GameObject::eState::Paused);
		}
		...
		AddGameObject(eLayerType::Monster, monster);
		monster->GetComponent<Transform>()->SetPosition(Vector3(500.0f, 0.0f, 450.0f));
		monster->GetComponent<Transform>()->SetVirtualZ(350.0f);
		monster->Initialize();
		monster->AddComponent<DragonSoldierScript>();
		monster->FindScript<DragonSoldierScript>()->EnterScene();
		...
		monster = new GameObject();
		AddGameObject(eLayerType::Monster, monster);
		monster->GetComponent<Transform>()->SetPosition(Vector3(1000.0f, 0.0f, 450.0f));
		monster->GetComponent<Transform>()->SetVirtualZ(350.0f);
		monster->Initialize();
		...
		monster = new GameObject();
		AddGameObject(eLayerType::Monster, monster);
		monster->GetComponent<Transform>()->SetPosition(Vector3(850.0f, 0.0f, 450.0f));
		monster->GetComponent<Transform>()->SetVirtualZ(200.0f);
		monster->Initialize();
		monster->AddComponent<DrakeSoldierScript>();
		monster->FindScript<DrakeSoldierScript>()->EnterScene();
	...
	void Dungeon_Hysmar_Entrance::Update()
	{
	...
	void Dungeon_Hysmar_Entrance::LateUpdate()
	{
		PlayScene::LateUpdate();
	}

	void Dungeon_Hysmar_Entrance::Render()
	{
		Scene::Render();
	}
	void Dungeon_Hysmar_Entrance::OnEnter()
	{
		PlayerScript* player = SceneManager::GetPlayer();
		if (player != nullptr)
		{
	...
	void Dungeon_Hysmar_Entrance::OnExit()
	{
		PlayerScript* player = SceneManager::GetPlayer();
		if (player != nullptr)
		{
