...
namespace hj
{
	...
	void Dungeon_Skasa_Entrance::Initialize()
	{
		...
		GameObject* gate = new GameObject();
		{
			...
			portal->SetDestination(L"Dungeon_Entrance_2", Vector2(850.0f, 250.0f));
		...
		gate = new GameObject();
		{
			...
			gate->AddComponent<CommonPortalScript>();
			PortalScript* portal = gate->AddComponent<PortalScript>();
			portal->SetDestination(L"Dungeon_Skasa", Vector2(300.0f, 250.0f));
			gate->SetState(GameObject::eState::Paused);
		}
		...
		monster->GetComponent<Transform>()->SetPosition(Vector3(700.0f, 0.0f, 450.0f));
		...
		monster = new GameObject();
		AddGameObject(eLayerType::Monster, monster);
		monster->GetComponent<Transform>()->SetPosition(Vector3(800.0f, 0.0f, 450.0f));
		monster->GetComponent<Transform>()->SetVirtualZ(200.0f);
		monster->Initialize();
		monster->AddComponent<DrakeSoldierScript>();
		monster->FindScript<DrakeSoldierScript>()->EnterScene();
	...
	void Dungeon_Skasa_Entrance::LateUpdate()
	{
		PlayScene::LateUpdate();
	}

	void Dungeon_Skasa_Entrance::Render()
	{
		Scene::Render();
	}
	void Dungeon_Skasa_Entrance::OnEnter()
	{
		PlayerScript* player = SceneManager::GetPlayer();
		if (player != nullptr)
		{
	...
	void Dungeon_Skasa_Entrance::OnExit()
	{
		PlayerScript* player = SceneManager::GetPlayer();
		if (player != nullptr)
		{
